...
public class ImageClassifierActivity extends Activity implements ImageReader.OnImageAvailableListener {
    ...
    public void onImageAvailable(ImageReader reader) {
        ...
        runOnUiThread(new Runnable() {
            @Override
            public void run() {
                onPictureTaken(byteArray, results.toString());
            }
        });

    }

    private void onPictureTaken(byte[] img, String results) {
        if (img != null) {
            final DatabaseReference log = mDatabase.getReference("imagens").push();
            // upload image to firebase

            String imageStr = Base64.encodeToString(img, Base64.NO_WRAP | Base64.URL_SAFE);

            log.child("imagem").setValue(imageStr);
            log.child("resultados").setValue(results);
        }
    }
